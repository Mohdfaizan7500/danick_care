...
const ProductDetails = () => {
  ...
  const [imageLoading, setImageLoading] = useState(false); // for product image
  const [imageError, setImageError] = useState(false);

  // State for part images: track loading and error per part (using id or index)
  ...
  const onRefresh = async () => {
    setRefreshing(true);
    if (product?.complaintId && product?.type === "Yes") {
      await fetchComplaintDetails(product.complaintId);
    }
    setRefreshing(false);
  };
  ...
  const getStatusColor = (status) => {
    if (status === 'market') return '#10B981';
    if (status === 'technician') return '#3B82F6';
      ...
      <ScrollView
        ...
        refreshControl={
          <RefreshControl
            ...
            colors={['#10B981']}
            tintColor="#10B981"
            title="Pull to refresh complaint details"
            titleColor="#666"
          />
        }
      >
              ...
              <View className="space-y-2">
                ...
                <View>
                  ...
                  <Text className="text-gray-800 text-sm">
                    {complaintDetails.service_address}
                  </Text>
                </View>
                ...
                <View className="flex-row justify-between items-center">
                  ...
                  <Text className="text-green-600 text-sm font-bold">
                    ₹{complaintDetails.total_paid_amt}
                  </Text>
                </View>
                ...
                <View className="flex-row justify-between items-center">
                  ...
                  <View
                    ...
                    style={{ backgroundColor: getStatusBadge(complaintDetails.status).bg }}
                  >
                    <Text
                      className="text-xs font-medium"
                      style={{ color: getStatusBadge(complaintDetails.status).color }}
                    >
                ...
                {/* Parts Used in Complaint - WITH IMAGES */}
                {complaintDetails.parts && complaintDetails.parts.length > 0 && (
                  <View className="mt-2 pt-2 border-t border-blue-200">
                    ...
                    {complaintDetails.parts.map((part, index) => {
                      const partKey = part.id?.toString() || `part-${index}`;
                      const isLoading = partImageLoading[partKey];
                      const hasError = partImageError[partKey];
                      return (
                        <View key={index} className="mb-3 p-2 bg-white rounded-lg">
                          <View className="flex-row">
                            {/* Part Image Thumbnail */}
                            <View className="w-16 h-16 bg-gray-100 rounded-lg mr-3 items-center justify-center overflow-hidden">
                              {isLoading && (
                                <ActivityIndicator size="small" color="#10B981" style={{ position: 'absolute', zIndex: 1 }} />
                              )}
                              {part.part_image && !hasError ? (
                                <Image
                                  source={{ uri: part.part_image }}
                                  className="w-full h-full"
                                  resizeMode="contain"
                                  onLoadStart={() => setPartImageLoading(prev => ({ ...prev, [partKey]: true }))}
                                  onLoadEnd={() => setPartImageLoading(prev => ({ ...prev, [partKey]: false }))}
                                  onError={() => {
                                    setPartImageError(prev => ({ ...prev, [partKey]: true }));
                                    setPartImageLoading(prev => ({ ...prev, [partKey]: false }));
                                  }}
                                />
                              ) : (
                                <View className="items-center justify-center">
                                  <Icon name="cube-outline" size={28} color="#9CA3AF" />
                                </View>
                              )}
                            </View>
                            {/* Part Details */}
                            <View className="flex-1">
                              <View className="flex-row justify-between items-start">
                                <Text className="text-gray-800 text-sm font-medium flex-1 mr-2">
                                  {part.part_name}
                                </Text>
                                <Text className="text-green-600 text-sm font-bold">
                                  ₹{part.part_price}
              ...
              <View className="flex-row justify-between">
                ...
                <Text
                  className={`font-medium ${
                    product.partAccept ? 'text-green-600' : 'text-yellow-600'
                  }`}
                >
